defmodule Xlsxir.Unzip do
  ...
  defp extract_from_zip(path, file_list, :memory) do
    :zip.extract(path, [{:file_list, file_list}, :memory])
  end

  defp extract_from_zip(path, file_list, {:file, dest_path}) do
    # Ensure dest_path exists with proper formatting
    dest_path =
      dest_path
    ...
    # Convert the dest_path to charlist to avoid Erlang 24+ compatibility issues
    # with string concatenation in the zip module
    cwd_option = {:cwd, String.to_charlist(dest_path)}

    case :zip.extract(path, [{:file_list, file_list}, cwd_option]) do
